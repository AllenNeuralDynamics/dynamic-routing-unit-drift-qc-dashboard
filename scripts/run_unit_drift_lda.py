...
def z_score(df: pl.DataFrame, columns: Iterable[str]) -> pl.DataFrame:
    return df.with_columns(
        *[(col - col.mean()) / col.std() for col in (pl.col(name) for name in columns)]
    )
...
def get_x_y(df: pl.DataFrame) -> tuple[npt.NDArray, npt.NDArray]:
    ...
    df = df.pipe(z_score, METRICS_TO_KEEP)
    x = get_x(df)
